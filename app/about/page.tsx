...
function StatCard({ stat, visible }: { stat: typeof defaultStats[0]; visible: boolean }) {
...
export default function AboutPage() {
  ...
  useEffect(() => {
    ...
    ]).then(([aboutData, settingsData, servicesData]) => {
      if (Object.keys(aboutData || {}).length > 0) setData(aboutData);
      if (settingsData) setSettings(settingsData);
      if (Array.isArray(servicesData) && servicesData.length > 0) setDbServices(servicesData);
      setLoading(false);
            ...
            <div className={styles.whoImgSide}>
              <div className={styles.whoImgWrap}>
                ...
                <div className={styles.expBadge}>
                  <div className={styles.expBadgeNum}>{data?.badgeNum}</div>
